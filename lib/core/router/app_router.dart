...
GoRouter buildAppRouter(AuthProvider authProvider) {
  return GoRouter(
    ...
    redirect: (context, state) {
      final loggingIn = state.matchedLocation.startsWith('/auth') || state.matchedLocation == '/splash';
      ...
      final protectedPrefixes = ['/etudiant', '/entreprise', '/administrateur'];
      if (!isAuthenticated && protectedPrefixes.any((p) => state.matchedLocation.startsWith(p))) {
        return '/';
      }
    ...
    routes: [
      ...
      GoRoute(
        path: '/auth',
        builder: (context, state) => AuthChoiceScreen(preselectedRole: state.uri.queryParameters['role']),
      ),
      GoRoute(
        path: '/auth/register',
        builder: (context, state) => RegisterScreen(role: state.uri.queryParameters['role'] ?? 'etudiant'),
      ),
      GoRoute(
        path: '/auth/login',
        builder: (context, state) => const LoginScreen(),
      ),
      GoRoute(path: '/etudiant', builder: (context, state) => const StudentDashboardScreen()),
      GoRoute(path: '/entreprise', builder: (context, state) => const EntrepriseDashboardScreen()),
      GoRoute(path: '/administrateur', builder: (context, state) => const AdministrateurDashboardScreen()),
    ],
  );
}
